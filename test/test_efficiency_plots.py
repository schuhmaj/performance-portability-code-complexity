...
def test_boxplot_uses_framework_colors_and_removes_descriptions():
    order = ["Cuda[Naive]", "Kokkos"]
    ...
    axis = figure.axes[0]
    expected = _framework_colors(order, remove_description=True)
    ...
    assert to_rgba(axis.patches[0].get_facecolor()) == to_rgba(expected[order[0]])
    assert to_rgba(axis.patches[1].get_facecolor()) == to_rgba(expected[order[1]])
    plt.close(figure)
